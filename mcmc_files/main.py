...
def main():
    ...
    x0 = [[M, mu, a, p0, e0, x0, dist, np.cos(qS), phiS, np.cos(qK), phiK] for n in range(Nchains)]  # start from truth
    ...
    start = time.time()
    niter = 2000
    c = S.run_mcmc(niter, pSwap=0.95, printN=5, multiproc=True, n0_swap=200)
    end = time.time()
